...
void ARifle::StartShooting()
{
	...
	{	
			...
			if (bIsAutomatic)
			{
				GetWorld()->GetTimerManager().SetTimer(ShootingTimerHandle, this, &ARifle::ShootingMechanism, WeaponInfo.FireRate, true, 0);
...
void ARifle::StartAiming() 
{
	Owner->PlayerActionState->bIsAiming = true;
	Owner->OnStartAiming();
}

void ARifle::StopAiming() 
{
...
void ARifle::StartSpeicalAction() 
{
	if (bIsAutomatic)
	{
		bIsAutomatic = false;
